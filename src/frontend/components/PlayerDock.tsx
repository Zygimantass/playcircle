...
import { classNames, Waveform, waveformClickPosition } from "./common";
...
export const PlayerDock = memo(function PlayerDock({
  ...
  return (
    <section className="grid h-full min-h-0 min-w-0 grid-cols-[minmax(0,1fr)_176px_minmax(0,1fr)] overflow-hidden bg-[#0b0c10]">
      <Deck
        deck="A"
        state={deckA}
...
function Deck({
  ...
  return (
    <section className="grid min-h-0 min-w-0 grid-rows-[52px_minmax(0,1fr)_112px] overflow-hidden border-r border-line last:border-r-0">
      <div className="flex min-w-0 items-center justify-between gap-3 border-b border-line bg-surface-1 px-4">
        <div className="flex min-w-0 items-center gap-2">
          <span className="grid size-7 place-items-center rounded-sm bg-accent font-mono text-[13px] font-bold text-bg">{deck}</span>
          <div className="min-w-0">
            <div className="overflow-hidden text-ellipsis whitespace-nowrap text-[14px] font-semibold text-text-1">{track.title}</div>
            <div className="overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[11px] text-text-2">{track.artist} · {track.bpm || "--"} BPM · {track.key}</div>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <span className="font-mono text-[10px]" style={{ color }}>{track.fileType}</span>
          <button
      ...
      <div className="grid min-h-0 min-w-0 grid-rows-[minmax(0,1fr)_42px] gap-2 overflow-hidden bg-[#08090c] p-3">
        <button
          className="relative min-h-0 min-w-0 overflow-hidden border border-line bg-[#07080b] text-left"
          onClick={(event) => {
            const bounds = event.currentTarget.getBoundingClientRect();
            const clickRatio = (event.clientX - bounds.left) / bounds.width;
            onSetPosition(deck, waveformClickPosition(track, state.position, clickRatio, "beat"));
          }}
          type="button"
        >
          <Waveform track={track} height="100%" playhead={state.position} showBeatGrid />
          <div className="pointer-events-none absolute left-2 top-2 rounded-sm bg-bg/70 px-1.5 py-px font-mono text-[11px] text-text-2">{formatClock(elapsed)} / {track.duration}</div>
        </button>
        <button
      ...
      <div className="grid min-w-0 grid-rows-[44px_1fr] gap-2 border-t border-line bg-gradient-to-b from-surface-1 to-[#0d0e12] px-4 py-3">
        ...
        <div className="grid min-w-0 grid-cols-2 items-center gap-4">
...
function MixerCenter({
  ...
  return (
    <div className="grid min-h-0 min-w-0 grid-rows-[52px_minmax(0,1fr)_126px] border-x border-line bg-surface-1">
